...
public class ItemListWidget extends AlwaysSelectedEntryListWidget<ItemListWidget.Entry> {
    ...
    public void setFocused(@Nullable Element focused) {
        ...
        if (size == 0) {
            this.parent.setCheckState(DropDownSelectListWidget.CheckState.UNCHECKED);
        } else if (size < this.children().size()) {
            this.parent.setCheckState(DropDownSelectListWidget.CheckState.INDETERMINATE);
        } else {
            this.parent.setCheckState(DropDownSelectListWidget.CheckState.CHECKED);
        }
    }
